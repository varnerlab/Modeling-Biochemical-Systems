### Stoichiometric modeling of metabolic pathways
...
There are only two terms that we need to consider for intracellular balances, the accumulation and generation terms. This is because there is no *convective* transport into or from cells, i.e., cell walls block convective transport of $$x_{j}$$ into or from the cell. Thus, the transport terms vanish $$\dot{m}_{X,in,j} = \dot{m}_{X,out,j} = 0$$ and the intracellular material balance around metabolite $$x_{j}$$ becomes:
$$
\dot{m}_{X,acc,j} = \dot{m}_{X,gen,j}\qquad{j=1,2,...,\mathcal{M}}
$$
The well-mixed assumption simplifies the accumulation term to:
$$
\dot{m}_{X,acc,j} = \frac{d}{dt}\int_{X}x_{j}dX\simeq X\frac{dx_{j}}{dt} + x_{j}\frac{dX}{dt}
$$
while the generation term becomes:
$$
\dot{m}_{X,gen,j} = \int_{X}\left(\displaystyle\sum_{r}^{\mathcal{R}}\sigma_{jr}v_{r} + \displaystyle\sum_{k}^{\mathcal{T}}\tau_{jk}q_{k}\right)dX \simeq \left(\displaystyle\sum_{r}^{\mathcal{R}}\sigma_{jr}v_{r} + \displaystyle\sum_{k}^{\mathcal{T}}\tau_{jk}q_{k}\right)X
$$
...
where $$\mathbf{x}^{*}$$ denotes the species abundance vector at steady-state.
...
The steady-state material balances have two sets of unknowns, states and the fluxes.
Without experimental measurements, we have way to estimate $$\mathbf{x}^{*}$$.
...
$$
where $$\mathbf{\Sigma}^{\#}$$ is a $$\mathcal{U}\times\mathcal{M}$$ *generalized* matrix inverse.

The matrix $$\mathbf{\Sigma}^{\#}$$ is the generalized inverse of the stoichiometric matrix corresponding to the unmeasured fluxes. The form of $$\mathbf{\Sigma}^{\#}$$ depends
